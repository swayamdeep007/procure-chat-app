...
window.toggleBulkMode = () => {
    ...
    if(isBulkMode) { btn.classList.add("active"); document.body.classList.add("bulk-active"); }
    else { btn.classList.remove("active"); document.body.classList.remove("bulk-active"); document.getElementById("bulk-action-bar").style.display = "none"; }
    renderPLList();
};
window.togglePLSelection = (plId, event) => {
    event.stopPropagation();
    if(selectedPLs.has(plId)) selectedPLs.delete(plId); else selectedPLs.add(plId);
    ...
    document.querySelectorAll(".pl-item").forEach(item => { if(item.getAttribute("data-id")===plId) item.querySelector(".bulk-check").checked = selectedPLs.has(plId); });
};
window.openBulkChat = () => {
    ...
    document.getElementById("chat-box").innerHTML = `<div style="text-align:center; padding:50px; color:#777;"><h3>📢 Bulk Broadcast Mode</h3><p>Sending to <b>${selectedPLs.size}</b> channels.</p></div>`;
};
...
window.sendBulkMessage = async () => {
    ...
    let newStatus = null; const lowerText = text.toLowerCase();
    if (lowerText.includes("#received")) newStatus = "Received"; else if (lowerText.includes("#tpi")) newStatus = "TPI"; else if (lowerText.includes("#escalated")) newStatus = "Escalated"; else if (lowerText.includes("#urgent")) newStatus = "Urgent";
    const batchLimit = 400; let batch = db.batch(); let opCount = 0;
    for(let plId of selectedPLs) {
        batch.set(db.collection("PLs").doc(plId).collection("Messages").doc(), { text, uid: CURRENT_USER_ID, userEmail: CURRENT_USER_EMAIL, userName: CURRENT_USER_NAME, userDesignation: CURRENT_USER_DESIGNATION, timestamp: Date.now(), isDeleted: false });
        opCount++;
        const updateData = { lastActivity: Date.now() };
        if(newStatus) updateData.status = newStatus;
        batch.update(db.collection("PLs").doc(plId), updateData);
        opCount++;
        if(opCount >= batchLimit) { await batch.commit(); batch = db.batch(); opCount = 0; }
    }
    if(opCount > 0) await batch.commit();
    progressEl.textContent = "✅ Done!"; document.getElementById("bulk-input").value = ""; setTimeout(() => { alert("Broadcast Sent!"); cancelBulkMode(); }, 1000);
};
...
window.handleBulkImageSelect = async () => {
    ...
    try {
        ...
        const batchLimit = 400; let batch = db.batch(); let opCount = 0;
        for(let plId of selectedPLs) {
            batch.set(db.collection("PLs").doc(plId).collection("Messages").doc(), { text: "", imageUrl: base64, uid: CURRENT_USER_ID, userEmail: CURRENT_USER_EMAIL, userName: CURRENT_USER_NAME, userDesignation: CURRENT_USER_DESIGNATION, timestamp: Date.now(), isDeleted: false });
            opCount++;
        ...
        if(opCount > 0) await batch.commit();
        alert("Image Broadcasted!"); cancelBulkMode();
...
window.openDashboard = async () => {
    ...
    let tableData = [];
    const poSnap = await db.collection("POs").get(); const poMap = {};
    ...
    allPLs.forEach(pl => {
        ...
        if(pl.status === 'Urgent' || pl.status === 'Escalated' || pl.status === 'Delayed') {
            ...
            tableData.push({ pl: pl.id, desc: pl.description, status: pl.status, supplier: poInfo.supplier || "-", qty: poInfo.qty || "-", dpdt: poInfo.dpdt || "-" });
        }
    });
    document.getElementById("dash-total").textContent = counts.Total; document.getElementById("dash-urgent").textContent = counts.Urgent; document.getElementById("dash-tpi").textContent = counts.TPI; document.getElementById("dash-received").textContent = counts.Received; document.getElementById("dash-esc").textContent = counts.Escalated;
    const tbody = document.getElementById("dashboard-table-body"); tbody.innerHTML = "";
    tableData.forEach(row => {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td><strong>${row.pl}</strong></td><td>${row.desc.substring(0,40)}...</td><td><span class="pl-status-badge" style="background:${row.status==='Urgent'?'#d32f2f':row.status==='Escalated'?'#b71c1c':'#f57c00'}; color:white;">${row.status}</span></td><td>${row.supplier}</td><td>${row.qty}</td><td>${row.dpdt}</td>`;
...
/* --- STANDARD CHAT & UTILS --- */
function formatTime(timestamp) { if (!timestamp) return ""; const date = new Date(timestamp); return `${date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })} ${date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`; }
function formatDateString(isoDate) { if(!isoDate) return ""; const date = new Date(isoDate); return isNaN(date) ? isoDate : date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }); }
function getUsername(email) { return email ? email.split('@')[0] : "User"; }
function compressAndConvertToBase64(file) { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.readAsDataURL(file); reader.onload = (e) => { const img = new Image(); img.src = e.target.result; img.onload = () => { const canvas = document.createElement('canvas'); const scale = 800/img.width; canvas.width = 800; canvas.height = img.height * scale; const ctx = canvas.getContext('2d'); ctx.drawImage(img,0,0,canvas.width,canvas.height); resolve(canvas.toDataURL('image/jpeg',0.7)); }; }; reader.onerror = reject; }); }
...
window.renderPLList = () => {
    const listEl = document.getElementById("pl-list"); listEl.innerHTML = "";
    ...
    allPLs.forEach(pl => {
        ...
        const div = document.createElement("div"); div.className = `pl-item status-${pl.status||'Normal'}`; div.setAttribute("data-id", pl.id);
        if(pl.id === selectedPL && !isBulkMode) div.classList.add("active");
        const av = pl.photoUrl || DEFAULT_AVATAR;
        div.innerHTML = `<input type="checkbox" class="bulk-check" ${selectedPLs.has(pl.id)?'checked':''} onclick="togglePLSelection('${pl.id}', event)"><img src="${av}" class="pl-avatar" onerror="this.src='${DEFAULT_AVATAR}'"><div class="pl-info"><span class="pl-status-badge" style="background:${pl.status==='Urgent'?'#d32f2f':pl.status==='TPI'?'#673ab7':'transparent'}; color:white">${pl.status!=='Normal'?pl.status:''}</span><span class="pl-number">${pl.id}</span><span class="pl-desc">${pl.description}</span></div>`;
        div.onclick = (e) => { if(e.target.type!=='checkbox') isBulkMode ? (div.querySelector('.bulk-check').click()) : selectPL(pl.id, pl.description, pl.status); };
        listEl.appendChild(div);
    });
};
window.filterPLs = () => renderPLList();
window.setFilter = (f) => { currentFilter = f; document.querySelectorAll('.chip').forEach(c=>c.classList.remove('active')); if(f==='All') document.getElementById('btn-all').classList.add('active'); else if(f==='Urgent') document.getElementById('btn-urgent').classList.add('active'); else if(f==='TPI') document.getElementById('btn-tpi').classList.add('active'); else if(f==='Received') document.getElementById('btn-received').classList.add('active'); renderPLList(); };

window.selectPL = (plId, desc, status) => {
    selectedPL = plId; document.getElementById("app-container").classList.add("chat-active");
    document.getElementById("standard-chat-header").style.display="flex"; document.getElementById("bulk-chat-header").style.display="none"; document.getElementById("bulk-controls").style.display="none"; document.getElementById("chat-controls").style.display="flex";
    document.getElementById("selected-pl-title").textContent = plId; document.getElementById("selected-pl-desc").textContent = desc; document.getElementById("selected-pl-status").textContent = status!=='Normal'?`[${status}]`:'';
    ...
    renderPLList();
    db.collection("PLs").doc(plId).get().then(d => { const l = document.getElementById("pos-list"); l.innerHTML=""; (d.data()?.relatedPOs||[]).forEach(p=>l.appendChild(new Option(p,p))); });
    db.collection("PLs").doc(plId).collection("Messages").orderBy("timestamp","asc").onSnapshot(s => {
        ...
        s.forEach(d => { const m=d.data(); if(m.isDeleted) return; const div = document.createElement("div"); div.className="message"+(m.uid===CURRENT_USER_ID?" you":"");
            div.innerHTML = `<span class="msg-sender">${m.userName}</span>${m.imageUrl?`<img src="${m.imageUrl}" class="msg-image" onclick="window.open('${m.imageUrl}')">`:''}${m.text?processTags(m.text):''}<span class="msg-time">${formatTime(m.timestamp)}</span>`;
            b.appendChild(div); });
...
window.sendMessage = async () => {
    ...
    let newStatus=null; const lower=text.toLowerCase(); if(lower.includes("#received")) newStatus="Received"; else if(lower.includes("#urgent")) newStatus="Urgent"; else if(lower.includes("#tpi")) newStatus="TPI";
    await db.collection("PLs").doc(selectedPL).collection("Messages").add({ text, uid: CURRENT_USER_ID, userEmail: CURRENT_USER_EMAIL, userName: CURRENT_USER_NAME, timestamp: Date.now(), isDeleted: false });
    const up={lastActivity: Date.now()}; if(newStatus) up.status=newStatus; await db.collection("PLs").doc(selectedPL).update(up);
...
window.uploadMasterCSV = () => { const f=document.getElementById("master-csv").files[0]; if(!f) return; Papa.parse(f, {header:false, complete: async r => { const b=db.batch(); r.data.forEach(row => { if(row.length<2)return; b.set(db.collection("PLs").doc(row[0]), {description:row[1], status:'Normal', lastActivity:Date.now()}, {merge:true}); }); await b.commit(); alert("Done"); closeSettings(); }}); };
window.exportChannelChat = async () => { if(!selectedPL) return; const s = await db.collection("PLs").doc(selectedPL).collection("Messages").orderBy("timestamp").get(); const d=[]; s.forEach(doc=>{ const m=doc.data(); d.push({Time:new Date(m.timestamp).toLocaleString(), User:m.userName, Message:m.text}); }); downloadCSV(Papa.unparse(d), "chat.csv"); };
